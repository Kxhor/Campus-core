...
if not _raw_origins:
    import warnings
    warnings.warn(
        "ALLOWED_ORIGINS is not set. Defaulting to same-origin only.",
        stacklevel=2
    )
    _allowed_origins = []
else:
...
def internal_server_error(e):
    # Log the error internally if a logger was configured
    return render_template('base.html', error='An internal server error occurred.'), 500
...
def register():
    if request.method == 'POST':
        ...
        email = safe_get('email', '').strip()
        password = request.form.get('password', '')
        reg_number = safe_get('reg_number', '').strip()
        department = safe_get('department', '').strip()
        role = 'student'
        phone = safe_get('phone', '').strip()
        year_raw = safe_get('year_of_study', '')
...
def admin_review_event(event_id):
    ...
    if decision == 'rejected':
        event.rejection_reason = request.json.get('reason', '') if request.is_json else safe_get('reason', '')
...
def create_event():
    user = get_current_user()
    if request.method == 'POST':
        title = safe_get('title', '').strip()
        event_date_str = safe_get('date', '')
        time_str = safe_get('time', '')
        venue = safe_get('venue', '').strip()
        description = safe_get('description', '').strip()
        category = safe_get('category', 'General')
        max_p_str = safe_get('max_participants', '')
        image_url = safe_get('image_url', '').strip()
...
def edit_event(event_id):
    ...
    if request.method == 'POST':
        ...
        if safe_get('remove_pdf') == '1' and event.pdf_file:
        ...
        event.title = safe_get('title', '').strip()
        try:
            event.date = datetime.strptime(safe_get('date'), '%Y-%m-%d').date()
        except (ValueError, TypeError):
            flash('Invalid date format.', 'danger')
            return redirect(url_for('edit_event', event_id=event.id))
        event.time = safe_get('time', '')
        event.venue = safe_get('venue', '').strip()
        event.description = safe_get('description', '').strip()
        event.category = safe_get('category', 'General')
        max_p_str = safe_get('max_participants', '')
        try:
            event.max_participants = int(max_p_str) if max_p_str else event.max_participants
        except (ValueError, TypeError):
            pass
        event.image_url = safe_get('image_url', '').strip()
        tags_raw = safe_get('tags', '')
        tag_list = [t.strip() for t in tags_raw.replace('\n', ',').split(',') if t.strip()]
...
def api_add_signatory(event_id):
    ...
    ordering = safe_get('ordering', 0, type=int)
...
def manage_attendance(event_id):
    ...
    if request.method == 'POST':
        for reg, user in registrations:
            status = safe_get(f'attendance_{user.id}', 'absent')
        ...
        for reg, student in registrations:
            if safe_get(f"attendance_{student.id}") == "present":
            ...
            socketio.emit('attendance:updated', {
                'event_id': event.id,
                'event_title': event.title,
                'status': safe_get(f'attendance_{student.id}', 'absent')
...
def admin_create_user():
    if request.method == 'POST':
        ...
        email = safe_get('email', '').strip()
        password = request.form.get('password', '')
        role = safe_get('role', 'student')
        college = safe_get('college', '').strip()
        department = safe_get('department', '').strip()
        phone = safe_get('phone', '').strip()
        year_raw = safe_get('year_of_study', '')
        year_of_study = int(year_raw) if year_raw and year_raw.isdigit() else None
        github_url = safe_get('github_url', '').strip()
        linkedin_url = safe_get('linkedin_url', '').strip()
        profile_photo_url = safe_get('profile_photo_url', '').strip()
        first_name = safe_get('first_name', '').strip()
        last_name = safe_get('last_name', '').strip()
        reg_number = safe_get('reg_number', '').strip()
        if not name or not email or not password:
...
def admin_edit_user(uid):
    ...
    if request.method == 'POST':
        ...
        u.name = safe_get('name', '').strip()
        u.first_name = safe_get('first_name', '').strip()
        u.last_name = safe_get('last_name', '').strip()
        new_email = safe_get('email', '').strip()
        ...
        u.email = new_email
        u.role = safe_get('role', 'student')
        u.college = safe_get('college', '').strip()
        u.department = safe_get('department', '').strip()
        u.reg_number = safe_get('reg_number', '').strip()
        u.phone = safe_get('phone', '').strip()
        year_raw = safe_get('year_of_study', '')
        u.year_of_study = int(year_raw) if year_raw and year_raw.isdigit() else None
        u.github_url = safe_get('github_url', '').strip()
        u.linkedin_url = safe_get('linkedin_url', '').strip()
        u.profile_photo_url = safe_get('profile_photo_url', '').strip()
        u.is_active = safe_get('is_active') == '1'
...
def admin_user_settings():
    ...
    if request.method == 'POST':
        ...
        settings.theme = safe_get('theme', 'light')
        settings.language = safe_get('language', 'english')
...
def admin_system_settings():
    if request.method == 'POST':
        ...
        default_language = safe_get('default_language', 'english')
...
def register_event(event_id):
    ...
    if event.start_time and event.end_time:
        ...
        if conflicts:
            ...
            confirm = safe_get('confirm_override')
            if not confirm:
...
def organizer_announcements():
    if request.method == 'POST':
        ...
        priority = safe_get('priority', 'info')
